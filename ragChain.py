...
def getRAGChain(retriever, llm):
    ...
    employee_prompt = ChatPromptTemplate.from_messages(
        [("system", """You are a Capability Maturity Model Integration (CMMI) expert with deep knowledge of the framework and its application. Your task is to come-up with what the team needs to answer and show evidences to the specific auditor questions and in general what the employee needs to prepare for passing the audit.
                     ...
                     **Output Requirements:**
                     - **Employee:** Offer a guide on what the employees should answer, and how employees should respond to the auditor's questions, ensuring that the Expectations align with CMMI expectations and are framed correctly.
...
def getRAGChain_Manual(retriever, llm):
    answer_prompt = ChatPromptTemplate.from_messages(
        [("system", """You are a Capability Maturity Model Integration (CMMI) expert with deep knowledge of the framework and its application. Your task is to analyze the following query based on the CMMI information provided in the PDF document. The query includes:
                     1. **Practice Area:** The specific domain within the CMMI framework.
                     2. **Level:** The CMMI maturity level relevant to the query.
    ...
    [("system", """You are a Capability Maturity Model Integration (CMMI) expert with deep knowledge of the framework and its application. Your task is to analyze the following query based on the CMMI information provided in the PDF document. The query includes:
                     ...
                     **Output Requirements:**
                     1. **Auditor Questions:** Formulate a set of around 3 to 5 probing questions that an auditor might ask employees based on the query, aiming to assess compliance or understanding.
                     2. **Employee Responses:** For each auditor question, provide a clear guide on how employees should respond, ensuring that the responses align with CMMI expectations. List the evidences that the employee is expected to show in response to each question.

                     Your final output should be comprehensive yet concise, ensuring that each component is uniquely tailored to the query while remaining accurate and aligned with the CMMI framework. Do not include the original query in your output.
                     context = {context}"""),
     ("human", "{input}")]
)
    ...
    # auditor_chain = create_stuff_documents_chain(llm, auditor_prompt)
    # employee_chain = create_stuff_documents_chain(llm, employee_prompt)
    combined_chain = create_stuff_documents_chain(llm, combined_prompt)
    ...
    chains = {
        ...
        # 'auditor': create_retrieval_chain(retriever, auditor_chain),
        # 'employee': create_retrieval_chain(retriever, employee_chain),
        'combined': create_retrieval_chain(retriever, combined_chain)
...
def getAnswerFromRAGChain_Manual(chains, prompt, output_type):
    if output_type not in chains:
        raise ValueError(f"Invalid output type: {output_type}")
